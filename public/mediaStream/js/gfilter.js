export default {
   ...
   */
	 ...
	 * @param l
	 */
   grayProcess: function (binaryData, l) {
		for (var i = 0; i < l; i += 4) {
			var r = binaryData[i]
			var g = binaryData[i + 1]
			var b = binaryData[i + 2]
	 ...
	 * 模糊
	 * @param tempCanvasData
	 */
	// blurProcess(context, canvasData) {
	blurProcess(tempCanvasData) {
		console.log('Canvas Filter - blur process')
		// var tempCanvasData = this.copyImageData(context, canvasData)
		// var tempCanvasData = canvasData
		var sumred = 0.0,
			sumgreen = 0.0,
			sumblue = 0.0
	...
	mirrorProcess: function (context, canvasData) {
		...
		for (
			var x = 0;
			x < tempCanvasData.width;
			x++ // column
		) {
			for (
				var y = 0;
				y < tempCanvasData.height;
				y++ // row
			) {
				// Index of the pixel in the array
				var idx = (x + y * tempCanvasData.width) * 4
